...
const ShukuyoCircleSVG: React.FC<{ honmei: ShukuyoType; size?: number }> = ({ honmei, size = 280 }) => {
  ...
  return (
    <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <Circle cx={cx} cy={cy} r={outerR} stroke={COLORS.primary} strokeWidth={2} fill="none" />
      <Circle cx={cx} cy={cy} r={innerR} stroke={COLORS.secondary} strokeWidth={1} fill="none" />
      ...
      <Circle cx={cx} cy={cy} r={30} fill={COLORS.white} stroke={COLORS.primary} strokeWidth={2} />
    </Svg>
  );
};
